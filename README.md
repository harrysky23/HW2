...
- **도커 컨테이너 실행**
  ```bash
  docker run -d --name mlops-sentiment-app -p 5001:5000 mlops-sentiment-app
  ```
- **실행 확인 후 중지 및 삭제**
  ```bash
...
3. **[GitHub Actions CD 단계]** 빌드가 성공하면, 미리 연결된 **내 로컬 PC(Self-hosted Runner) 화면**으로 명령이 하달되어 기존 컨테이너를 삭제하고, 최신 이미지를 다운 받아 포트 5001번으로 재가동합니다.
...
4. 완료 즉시, 브라우저에서 `http://localhost:5001` 접속이 제대로 되는지, 혹은 UI 상에서 글을 입력했을 때 분석 결과가 방금 바꾼 코드대로 (`AI_prediction` 형태로) 정상 표출되는지 확인합니다. 이것이 완벽하게 이루어졌다면 **자동 대응형 MLOps 파이프라인 구축 성공**입니다.
